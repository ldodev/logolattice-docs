...
<Tip>
  The `mcp` and `add-mcp` options are especially powerful - they let developers connect your entire docs site to any MCP-compatible AI assistant, so it can answer questions about your product
  directly.
</Tip>
...
<Note>The contextual toolbar is only visible on preview and production deployments, not in `mint dev` local preview.</Note>
